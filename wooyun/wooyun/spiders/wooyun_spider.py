...
class WooyunSpider(Spider):
    ...
    def parse(self , response):
        ...
        #Request(list_url,callback = self.parse_list)
    ...
    def parse_article(self , response):
        ...
        item['bug_title'] = sel.xpath("//h3[@class='wybug_title']/text()").extract()[0].replace('\t','')
        item['open_date'] = sel.xpath("//h3[@class='wybug_date']/text()").re("[\d+]{4}-[\d+]{2}-[\d+]{2} [\d+]{2}:[\d+]{2}")
        item['bug_type'] = sel.xpath("//h3[@class='wybug_type']/text()").extract()[0].replace('\t', '')
        #item['bug_id'] = sel.xpath("//div[@class='content']/h3/a[contains(@href,'bugs/wooyun')]/text()").extract()[0]
        item['bug_id'] = response.url.split('/')[4] 
        item['author'] =  sel.xpath("//h3[@class='wybug_author']/a/text()").extract() 
